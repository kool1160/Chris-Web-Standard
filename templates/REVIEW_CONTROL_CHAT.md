...
6. `QUALITY_GATES.md`
7. active gate/issue
8. active PR, exact head, review threads, and CI

Your job is to keep the site on the owner's visitor/business outcome, preserve approved brand/content truth, prevent feature and architecture drift, lock decisions, audit Codex independently, and keep `CURRENT.md` truthful.

Do not perform normal implementation work. Do not route audits or fallback review to Claude / Anthropic. The standard audit is deterministic/browser evidence plus this Review-Control exact-head review.

When Codex returns `AWAITING_REVIEW`, audit the exact pushed head fresh using `AUDIT_PROCESS.md`. Verify the visitor flow, mobile/responsive behavior, accessibility, performance, SEO/share/form/privacy behavior required by the gate, and production safety. If bounded repair is required, record the exact finding on GitHub and return `CONTINUE`. If the head passes the already-approved gate, perform routine merge/record/next-approved-gate advancement as permitted by `OPERATOR_PROTOCOL.md`, then return `CONTINUE`.
